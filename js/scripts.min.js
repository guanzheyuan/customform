...
function loadJson(){
        ...
        for(var i=0;i<allDom.length;i++){
            ...
            if(allDom[i].type == "text"){
                ...
                if(' '!=allDom[i].attributes.orgname.value){
                    for(var j = 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            formInputElements.label = window.labelArr[j].text;
                            formInputElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"))*2;
                        }
                    }
                }
                ...
                if(' '!=allDom[i].attributes.orgname.value){
                    for(var j = 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            ...
                            formSelectElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"))*2;
                        }
                    }
                }
            ...
            if(allDom[i].type == "textarea"){
                ...
                if(allDom[i].attributes.orgname && ' '!=allDom[i].attributes.orgname.value){
                    for(var j = 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            formTextareaElements.label = window.labelArr[j].text;
                            formTextareaElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"))*2;
                        }
                    }
                }
                ...
                if(allDom[i].attributes.orgname && ' '!=allDom[i].attributes.orgname.value){
                    for(var j= 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            ...
                            formRadioElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"));
                        }
                    }
                }
                ...
                if(allDom[i].attributes.orgname && ' '!=allDom[i].attributes.orgname.value){
                    for(var j = 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            ...
                            formCheckBoxElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"));
                        }
                    }
                }
                ...
                if(allDom[i].attributes.orgname && ' '!=allDom[i].attributes.orgname.value){
                    for(var j = 0;j<window.labelArr.length;j++){
                        if(window.labelArr[j].value == allDom[i].attributes.orgname.value){
                            ...
                            formTimeElements.labelGridCol = parseInt($("#"+allDom[i].attributes.orgname.value).attr("col"))*2;
                        }
                    }
                }
...
function clickFix(obj){
   if($(obj).parent().find(".editElement").length > 0){
        ...
        if('grid-layout' == dataType && isLayoutClick){
            ...
            var radioHtml = $(element.parent()).find("input[type=radio]")
            console.log(radioHtml.length);
            if(radioHtml.length > 0){
                ...
                for(var i = 0;i<radioHtml.length > 0;i++){
                    if($(radioHtml[i]).attr('value') == undefined && $(radioHtml[i]).attr("valueName") == undefined){
...
function onloadDateTimeModal(){
    $("#editorDateTimeModal_ ").find(".orgname").bind("input propertychange",function(event){
        ...
        if($(this).val().length >15 &&$(this).val().length <= 20){
            $(dom).css('width','70%');
        }
...
function onloadCheckBoxModal(){
    $("#editorCheckBoxModal_ ").find(".orgname").bind("input propertychange",function(event){
        ...
        $(dom.parent().parent()).find("label").html($(this).val());
        $(dom).attr('orgname',$(this).val())
    });
...
function onloadInputModal(){
    $("#editorInputModal_ ").find(".orgname").bind("input propertychange",function(event){
        ...
        if($(this).val().length <= 10){
            $(dom).css('width','84%');
            $("#editorInputModal_ .layoutCol").val($(dom.prev()).css('width').replace(/px/ig, ""));
        }
        if($(this).val().length >10 &&$(this).val().length <= 15){
            $(dom).css('width','77%');
            $("#editorInputModal_ .layoutCol").val($(dom.prev()).css('width').replace(/px/ig, ""));
...
function checkCheckboxOption(obj){
    $("#editorCheckBoxModal_").delegate(".addCheckboxOption", "change", function (e) {
        var dom = window.checkBoxDom_;
        ...
        if(e.target.checked){
            ...
            if(''!=valueName && ''!=value){
                var html = '<input type="radio" value='+value+' label='+valueName+'>'+valueName+'</input>';
                $(dom.parent()).html(html);
            }
        }else{
            var $valueName =  $(e.target).parent().parent().parent();
            var valueName = $valueName.find("[name=valueName]").val();
            var radioHtml = $(dom).find('input[type=radio]');
            if(radioHtml.length > 0){
...
$(document).ready(function () {
    ...
    $(".sidebar-nav .lyrow").draggable({
        ...
        stop: function (e, t) {
            ...
            //拖拽后增加标记
            console.log($(".demo .addElement").length);
            if($(".demo .addElement").length > 0){
                $(".demo .addElement").addClass('editElement');
                $(".demo .addElement").find(".view").children().attr("id",Math.floor(Math.random()*10000))
            }
        }
    });
    ...
    $("#editorLayout .layoutCol").bind("change",function(event){
       ...
       var col = value;
       var ratio = 12/value;
       var domId = $(domObj.parent()).attr("id");
       var html = '';
       for(var i=0;i<col;i++){
        html+='<div class="col-md-'+ratio+' column" type="grid-layout" ></div>'
       }
       $("#"+domId).html(html);
       window.selectDom = $("#"+domId).children();
       $(".demo, .demo .column").sortable({
            connectWith: ".column",
            opacity: .35,
            handle: ".drag"
